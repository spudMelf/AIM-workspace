...
from predict import get_prediction, get_prediction_w_noise
...
UPLOAD_FOLDER = r"/Users/eamon/Desktop/AIM-workspace/deepfake-project/catchFake/flask_server/app/user_files"
...
@app.route('/predict_w_noise', methods=['POST'])
def predict_w_noise():
    if request.method == 'POST':
        file = request.files['file']
    if file and allowed_file(file.filename):
        ...
        with open(app.config['UPLOAD_FOLDER'] + "/temp.png", 'rb') as image:
            img_bytes = image.read()
            class_id, class_name = get_prediction_w_noise(image_bytes=img_bytes)
            image.close()
            clear_directory(app.config['UPLOAD_FOLDER'])
            return jsonify({'class_id': class_id, 'class_name': class_name})
